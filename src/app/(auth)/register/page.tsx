/**
 * ============================================================================
 * PÁGINA DE REGISTRO - VIRUCHECK
 * ============================================================================
 * - Registro manual con validación de seguridad de contraseña.
 * - Soporte para Google con intercepción de perfil nuevo para asignar contraseña propia.
 * - Diseño moderno, responsivo (móvil y desktop) con efectos glassmorphism.
 */
...
export default function RegisterPage() {
  ...
  // Estados específicos para el flujo intermedio de Google en usuarios nuevos
  const [needsGooglePasswordSetup, setNeedsGooglePasswordSetup] = useState(false);
  ...
  /**
   * Registro con Google optimizado con validación de perfil nuevo a través del contexto actualizado
   */
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (isNewUser) {
        // Es un usuario NUEVO de Google: lo detenemos para pedirle contraseña doble
  ...
  /**
   * Guarda los datos del usuario nuevo de Google junto con su contraseña con doble confirmación
   */
  const handleSaveGoogleNewUser = async (e: React.FormEvent) => {
    ...
    if (googlePassword !== confirmGooglePassword) {
      setErrorMessage("Las contraseñas no coinciden. Por favor, asegúrate de escribirlas bien en ambos campos.");
      return;
    }
    ...
    try {
      ...
      await setDoc(userDocRef, {
        ...
        customPassword: googlePassword, // Almacenado de forma segura en Firestore
  ...
  return (
    <div className="relative min-h-[100dvh] w-full flex items-center justify-center p-4 sm:p-6 bg-slate-950 overflow-hidden">
      <div className="absolute top-[-15%] right-[-10%] w-[350px] sm:w-[450px] h-[350px] sm:h-[450px] bg-blue-600/15 rounded-full blur-[120px] pointer-events-none" />
      <div className="absolute bottom-[-15%] left-[-10%] w-[350px] sm:w-[450px] h-[350px] sm:h-[450px] bg-emerald-600/10 rounded-full blur-[120px] pointer-events-none" />
      ...
      <div className="relative w-full max-w-[420px] rounded-3xl border border-slate-800 bg-slate-900/80 p-6 sm:p-8 shadow-2xl backdrop-blur-xl">
        <div className="flex flex-col items-center text-center space-y-3 mb-6">
          <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-tr from-blue-600 to-cyan-500 text-white shadow-lg shadow-blue-500/25">
            {needsGooglePasswordSetup ? <Sparkles className="h-6 w-6" /> : <Wallet className="h-6 w-6" />}
          </div>

          <div className="space-y-1">
            <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-white">
              {needsGooglePasswordSetup ? "Asegura tu Cuenta" : "Crear Cuenta"}
            </h1>
            <p className="text-xs text-slate-400">
              {needsGooglePasswordSetup
                ? "Es tu primera vez con Google. Asigna una contraseña para tu cuenta."
        ...
        {/* FLUJO 1: CONFIGURACIÓN DE CONTRASEÑA DOBLE PARA USUARIOS NUEVOS DE GOOGLE */}
        {needsGooglePasswordSetup ? (
          <form onSubmit={handleSaveGoogleNewUser} className="space-y-4 text-xs">
            <div className="space-y-1.5">
              <Label className="text-slate-300 font-bold flex items-center gap-1.5">
                <User className="h-3.5 w-3.5 text-cyan-400" /> Nombre Completo
              </Label>
              <Input
                type="text"
                required
                ...
                className="h-11 rounded-2xl border-slate-800 bg-slate-950/70 text-slate-100 text-xs px-4"
              />
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                className="h-11 rounded-2xl border-slate-800 bg-slate-950/70 text-slate-100 text-xs px-4 font-mono"
              />
            </div>
            ...
            <Button
              type="submit"
              disabled={loading}
              className="h-11 w-full gap-2 rounded-2xl bg-gradient-to-r from-blue-600 to-cyan-600 font-bold text-xs text-white shadow-lg shadow-blue-600/25 hover:brightness-110 transition-all mt-2 cursor-pointer"
            >
              {loading ? (
                <>
          ...
          /* FLUJO 2: REGISTRO NORMAL MANUAL O CON GOOGLE */
          <>
            <form onSubmit={handleSubmit} className="space-y-3.5">
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    onChange={(e) => setName(e.target.value)}
                    required
                    className="h-11 rounded-2xl border-slate-800 bg-slate-950/70 pl-10 text-xs text-slate-100 placeholder:text-slate-600 focus:border-blue-500"
                  />
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    required
                    className="h-11 rounded-2xl border-slate-800 bg-slate-950/70 pl-10 pr-10 text-xs text-slate-100 placeholder:text-slate-600 focus:border-blue-500"
                  />
                  <button
                    type="button"
              ...
              <Button
                ...
                className="h-11 w-full gap-2 rounded-2xl bg-blue-600 text-xs font-bold text-white shadow-lg shadow-blue-600/20 hover:bg-blue-500 active:scale-[0.99] transition-all mt-1 cursor-pointer"
              >
                {loading ? (
                  <>
                    <Loader2 className="h-4 w-4 animate-spin" />
            ...
            <Button
              ...
              disabled={loading || googleLoading}
              className="h-11 w-full gap-2.5 rounded-2xl border-slate-800 bg-slate-950/60 text-xs font-semibold text-slate-200 hover:bg-slate-800 hover:text-white cursor-pointer"
            >
              {googleLoading ? (
