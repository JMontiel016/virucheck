...
export default function LoginPage() {
  ...
  // Estados del flujo PIN de Recuperación
  const [step, setStep] = useState<"login" | "request_pin" | "enter_pin" | "new_password">("login");
  ...
  // 2. Validar solo el PIN (Teclado numérico)
  ...
  // 3. Guardar la nueva contraseña e iniciar sesión
  const handleSaveNewPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setAlertInfo({
        type: "success",
        text: "¡Contraseña actualizada con éxito! Entrando a la aplicación...",
      });
  ...
  return (
    <div className="relative min-h-[100dvh] w-full flex items-center justify-center p-4 sm:p-6 bg-slate-950 overflow-hidden">
      <div className="absolute top-[-15%] left-[-10%] w-[450px] h-[450px] bg-blue-600/15 rounded-full blur-[120px] pointer-events-none" />
      <div className="absolute bottom-[-15%] right-[-10%] w-[450px] h-[450px] bg-emerald-600/10 rounded-full blur-[120px] pointer-events-none" />
      ...
      <div className="relative w-full max-w-[420px] rounded-3xl border border-slate-800 bg-slate-900/80 p-6 sm:p-8 shadow-2xl backdrop-blur-xl">
        <div className="flex flex-col items-center text-center space-y-3 mb-6">
          <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-tr from-blue-600 to-cyan-500 text-white shadow-lg shadow-blue-500/25">
            {step === "login" ? <Wallet className="h-6 w-6" /> : <KeyRound className="h-6 w-6" />}
          </div>
        ...
        {step === "login" && (
          <form onSubmit={handleSubmitLogin} className="space-y-4">
            ...
            <Button
              ...
              className="h-11 w-full gap-2 rounded-xl bg-blue-600 text-xs font-bold text-white shadow-lg shadow-blue-600/20 hover:bg-blue-500 active:scale-[0.99] transition-all"
            >
              {loading ? (
                <>
        ...
        {step === "request_pin" && (
          <form onSubmit={handleRequestPin} className="space-y-4">
            ...
            <Button
              type="submit"
              disabled={loading}
              className="h-11 w-full gap-2 rounded-xl bg-blue-600 text-xs font-bold text-white hover:bg-blue-500"
            >
              {loading ? (
                <>
                  <Loader2 className="h-4 w-4 animate-spin" />
        ...
        {/* 3. MODO INGRESAR SOLO EL PIN (Teclado puramente numérico) */}
        {step === "enter_pin" && (
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  required
                  className="h-12 text-center font-mono tracking-[0.4em] text-xl font-bold rounded-xl border-slate-800 bg-slate-950/70 text-blue-400 focus:border-blue-500"
                />
              </div>
            </div>
        ...
        {step === "new_password" && (
          <form onSubmit={handleSaveNewPassword} className="space-y-4">
            <div className="space-y-1.5">
              <Label className="text-xs font-medium text-slate-300">Ingresa tu nueva contraseña</Label>
              <div className="relative">
                <Lock className="absolute left-3.5 top-3 h-4 w-4 text-slate-500" />
                <Input
                  type={showPassword ? "text" : "password"}
            ...
            <Button
              type="submit"
              disabled={loading}
              className="h-11 w-full gap-2 rounded-xl bg-emerald-600 text-xs font-bold text-white hover:bg-emerald-500"
            >
              {loading ? (
                <>
                  <Loader2 className="h-4 w-4 animate-spin" />
